...
**Response**:The gentoo species has the longest flipper length.
...
**Response**:The box plot is easier to understand and interprete.You can
easily tell each species and it’s flipper length.
...
**Response**:The activity02-ggplot2-intro.md is easier to read.The mark
down file also looks more proffesional
...
**Response**: I prefer the jitterplot laid over the box plot because
then you can see the distribution of the points unlike when the boxplot
is laid over the jitterplot.
...
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species )) +
  geom_boxplot() + 
geom_jitter(mapping = aes(x = flipper_length_mm, y = species, fill = species ))
```
...
**Response**: Changing the location of the aesthetic mappings within the
code changes the final outlook of the plot.
...
**Response**: the type of species and the island that they are found may
have led to the peaks in the graphs because the two factors also lead to
longer bill lengths as seen in the box plots.
...
**Response**:flipper length has a positive relationship to bodymass i.e
the longer the flipper length the higher the body mass.
...
**Response**:The Gentoo species has generally longer flipper lengths and
corresponding higher body mass than the rest.
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = body_mass_g, colour = species, shape = species), size = "3" , alpha = "0.6") +
geom_point()
```
...
**Response**:The size aesthetic increases the size of the points or
shape making them more visible while the alpha makes a difference in the
level or degree of shading.
